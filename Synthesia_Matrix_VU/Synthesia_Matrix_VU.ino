/*
VU Meter by Overhauser
Based on code developed by Adafruit Industries
...
void stripOff();
void pixelOff(int aPixel);
void solidColor(uint32_t c);
void setPixelRange(uint32_t c, int pStart, int pEnd);
void setColumn(uint32_t c, int column, int height);
void pixelSet(int x, int y, uint32_t c);
uint32_t Wheel(uint16_t WheelPos);
uint32_t miniWheel(uint16_t WheelPos);
uint32_t jumpWheel(uint16_t WheelPos);
...
void setup() {
  ...
  ADMUX  = 0x40; // Channel sel, right-adj, use AREF pin
...
void loop() 
{
  ...
  for(x=0; x<8; x++) {
    ...
    int outputIntensity = map(c, 0, 8, -1, 3);
    // Both are excellent settings!
    setColumn(jumpWheel(c), x, outputIntensity);
    //setColumn(miniWheel(outputIntensity), x, outputIntensity);
...
void pixelOff(int aPixel) {
  ...
  strip.setPixelColor(aPixel, strip.Color(0,0,0));
  strip.show();
...
uint32_t miniWheel(uint16_t WheelPos)
{
  ...
  {
    case 0:
      r = 7 - WheelPos % 8; // red down
      g = WheelPos % 8;       // green up
      b = 0;                    // blue off
      break;
    case 1:
      g = 7 - WheelPos % 8; // green down
      b = WheelPos % 8;       // blue up
      r = 0;                    // red off
      break;
    case 2:
      b = 7 - WheelPos % 8; // blue down
      r = WheelPos % 8;       // red up
      g = 0;                    // green off
      break;
  }
...
uint32_t jumpWheel(uint16_t aPos)
{
  byte r, g, b;
  
  uint16_t WheelPos = aPos*45;
...
void setColumn(uint32_t c, int column, int height) {
  ...
  for (int i=0; i<4; i++) 
    ...
    // Erase old pixels if needed
    pixelSet(column, i, offColor);      
    }
    //delay(10);
    //strip.setPixelColor(column+(i*8), c);
    }
